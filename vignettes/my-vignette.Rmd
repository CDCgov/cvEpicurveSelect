...
For the CITS, we will be using a method similar to the one described by @Bernal2018 with the equation below. 
...
* $T_{int}$:   dummy variable indicating the pre- or post-intervention period
...
dat <- dat%>% ungroup()%>%
        mutate(Tint = ifelse(T<t_int,0,(T-t_int)+1), 
               T_G  =T*G,
               X_G  =X*G,
               Tint_G=Tint*G)

fit <- nlme::gls(
      Yt ~ T +X+Tint+G+T_G+X_G+Tint_G,
      data = dat,
